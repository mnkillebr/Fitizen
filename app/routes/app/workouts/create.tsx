            ...
            <div className="flex flex-col gap-y-2 xl:grid xl:grid-cols-2 xl:gap-4 snap-y snap-mandatory overflow-y-auto px-0.5 pb-1 text-slate-900">
              {exercises.map((ex_item) => (
                <Exercise
                  ...
                  selectCount={workoutCards.reduce((result, curr) => {
                  ...
                  }, []).map(sel_ex => sel_ex.id.split("-")[0]).filter(id => id === ex_item.id).length}
                  selected={workoutCards.reduce((result, curr) => {
                    let resultArr = result
                    if (curr.exercises) {
                      return resultArr.concat(curr.exercises)
                    } else {
                  ...
                  }, []).map(sel_ex => sel_ex.id.split("-")[0]).includes(ex_item.id)}
                />
              ))}
            </div>
          </motion.div>
